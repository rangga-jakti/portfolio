...
import { trackViewProjects, trackContactCTA } from "@/lib/analytics";
...
export default function Hero() {
  ...
  return (
    <section className="relative min-h-screen flex items-center overflow-hidden bg-grid bg-grid-sm" id="home">
      <ParticleField />
      <div className="scanline opacity-30" />

      <motion.div style={{ x: orbX, y: orbY }}
        className="orb w-[700px] h-[700px] top-[-15%] left-[-10%] opacity-25"
        animate={{ background: ["radial-gradient(circle, rgba(0,255,224,0.18) 0%, transparent 70%)", "radial-gradient(circle, rgba(0,184,255,0.18) 0%, transparent 70%)", "radial-gradient(circle, rgba(0,255,224,0.18) 0%, transparent 70%)"] }}
        transition={{ duration: 7, repeat: Infinity, ease: "easeInOut" }}
      />
      <motion.div style={{ x: orb2X, y: orb2Y }}
        className="orb w-[500px] h-[500px] bottom-[-10%] right-[5%] opacity-15"
        animate={{ background: ["radial-gradient(circle, rgba(255,184,0,0.10) 0%, transparent 70%)", "radial-gradient(circle, rgba(139,92,246,0.10) 0%, transparent 70%)", "radial-gradient(circle, rgba(255,184,0,0.10) 0%, transparent 70%)"] }}
      ...
      <div className="max-w-6xl mx-auto px-6 pt-24 pb-16 w-full relative z-10">
        <div className="flex flex-col lg:flex-row items-center justify-between gap-10 lg:gap-8">
          ...
          {/* LEFT */}
          <motion.div variants={containerVariants} initial="hidden" animate="visible" className="flex-1 max-w-2xl w-full">
            <motion.div variants={itemVariants} className="mb-6">
              <div className="inline-flex items-center gap-2 px-3 py-1.5 border border-cyan-glow/30 bg-cyan-dim rounded-full">
                ...
                <span className="font-mono text-cyan-glow text-xs tracking-widest">Available for remote work</span>
              </div>
            </motion.div>

            <motion.div variants={itemVariants} className="mb-2">
              <span className="font-mono text-text-secondary text-sm tracking-widest">Hi, I&apos;m</span>
            </motion.div>

            <motion.h1 variants={itemVariants} className="font-syne font-extrabold text-5xl md:text-7xl text-text-primary leading-none mb-3">
              {siteConfig.name.split(" ")[0]}
              <br />
              <span className="gradient-text">{siteConfig.name.split(" ")[1]}.</span>
            </motion.h1>

            <motion.div variants={itemVariants} className="font-syne font-bold text-xl md:text-3xl text-text-secondary leading-tight mb-4 h-10 md:h-12 flex items-center">
            ...
            <motion.div variants={itemVariants} className="flex items-center gap-2 mb-5">
              ...
              <span className="font-outfit text-text-secondary text-sm">{siteConfig.locationFull}</span>
            </motion.div>

            <motion.p variants={itemVariants} className="font-outfit text-text-secondary text-base leading-relaxed max-w-xl mb-8">
              {siteConfig.heroDescription}
            </motion.p>

            <motion.div variants={itemVariants} className="flex flex-wrap items-center gap-4 mb-10">
              {/* View My Projects — tracked */}
              <a
                href="#projects"
                onClick={trackViewProjects}
            ...
            <motion.div variants={itemVariants} className="flex flex-wrap gap-8 pt-6 border-t border-border/40">
              {heroStats.map(({ number, label }) => (
                <div key={label} className="flex flex-col">
                  <span className="font-syne font-bold text-2xl text-cyan-glow">{number}</span>
                  <span className="font-outfit text-xs text-text-muted uppercase tracking-wider mt-0.5 max-w-[120px]">{label}</span>
                </div>
              ))}
            </motion.div>
          </motion.div>
